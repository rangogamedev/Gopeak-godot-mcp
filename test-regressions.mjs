...
// resource_tools.set_tilemap_cells must paint + persist cells for BOTH the
// Godot-4 TileMapLayer node (no layer arg) and the deprecated multi-layer
// TileMap node (layer arg). Reloads with CACHE_MODE_IGNORE so the assertion
// reads the freshly-saved .tscn off disk rather than a stale cached PackedScene.
function testTilemapCellsRegression() {
  const godotPath = resolveGodotPath();
  if (!godotPath) {
    console.log('tilemap cells regression skipped (Godot not found)');
    return;
  }

  const projectDir = mkdtempSync(join(tmpdir(), 'gopeak-tilemap-'));
  try {
    mkdirSync(join(projectDir, 'addons', 'godot_mcp_editor', 'tools'), { recursive: true });
    cpSync('src/addon/godot_mcp_editor/tools/resource_tools.gd', join(projectDir, 'addons', 'godot_mcp_editor', 'tools', 'resource_tools.gd'));

    writeFileSync(join(projectDir, 'project.godot'), `; Engine configuration file.\n; It's best edited using the editor.\nconfig_version=5\n\n[application]\nconfig/name="GopeakTilemapRegression"\n`);

    writeFileSync(join(projectDir, 'runner.gd'), `extends SceneTree\n\nfunc _fail(message: String) -> void:\n\tprinterr(message)\n\tquit(1)\n\nfunc _make_tileset() -> TileSet:\n\tvar ts := TileSet.new()\n\tvar atlas := TileSetAtlasSource.new()\n\tvar img := Image.create(16, 16, false, Image.FORMAT_RGBA8)\n\timg.fill(Color.WHITE)\n\tatlas.texture = ImageTexture.create_from_image(img)\n\tatlas.texture_region_size = Vector2i(16, 16)\n\tatlas.create_tile(Vector2i(0, 0))\n\tts.add_source(atlas, 0)\n\treturn ts\n\nfunc _save_scene(root: Node, path: String) -> bool:\n\tvar packed := PackedScene.new()\n\tif packed.pack(root) != OK:\n\t\treturn false\n\treturn ResourceSaver.save(packed, path) == OK\n\nfunc _reload(path: String) -> Node:\n\tvar packed := ResourceLoader.load(path, "PackedScene", ResourceLoader.CACHE_MODE_IGNORE) as PackedScene\n\tif packed == null:\n\t\treturn null\n\treturn packed.instantiate()\n\nfunc _init() -> void:\n\tvar layer_root := Node2D.new()\n\tlayer_root.name = "Root"\n\tvar tml := TileMapLayer.new()\n\ttml.name = "Layer"\n\ttml.tile_set = _make_tileset()\n\tlayer_root.add_child(tml)\n\ttml.owner = layer_root\n\tif not _save_scene(layer_root, "res://layer.tscn"):\n\t\t_fail("failed to save TileMapLayer scene")\n\t\treturn\n\tlayer_root.queue_free()\n\n\tvar map_root := Node2D.new()\n\tmap_root.name = "Root"\n\tvar tm := TileMap.new()\n\ttm.name = "Map"\n\ttm.tile_set = _make_tileset()\n\twhile tm.get_layers_count() < 1:\n\t\ttm.add_layer(-1)\n\tmap_root.add_child(tm)\n\ttm.owner = map_root\n\tif not _save_scene(map_root, "res://map.tscn"):\n\t\t_fail("failed to save TileMap scene")\n\t\treturn\n\tmap_root.queue_free()\n\n\tvar res_tools = load("res://addons/godot_mcp_editor/tools/resource_tools.gd").new()\n\n\tvar layer_result: Dictionary = res_tools.set_tilemap_cells({\n\t\t"scenePath": "layer.tscn",\n\t\t"tilemapNodePath": "Layer",\n\t\t"cells": [{"coords": {"x": 1, "y": 2}, "sourceId": 0, "atlasCoords": {"x": 0, "y": 0}}]\n\t})\n\tif not layer_result.get("ok", false):\n\t\t_fail("TileMapLayer set_tilemap_cells failed: %s" % JSON.stringify(layer_result))\n\t\treturn\n\n\tvar map_result: Dictionary = res_tools.set_tilemap_cells({\n\t\t"scenePath": "map.tscn",\n\t\t"tilemapNodePath": "Map",\n\t\t"layer": 0,\n\t\t"cells": [{"coords": {"x": 3, "y": 4}, "sourceId": 0, "atlasCoords": {"x": 0, "y": 0}}]\n\t})\n\tif not map_result.get("ok", false):\n\t\t_fail("TileMap set_tilemap_cells failed: %s" % JSON.stringify(map_result))\n\t\treturn\n\n\tvar missing: Dictionary = res_tools.set_tilemap_cells({\n\t\t"scenePath": "layer.tscn",\n\t\t"tilemapNodePath": "DoesNotExist",\n\t\t"cells": []\n\t})\n\tif missing.get("ok", true):\n\t\t_fail("expected ok:false for missing node: %s" % JSON.stringify(missing))\n\t\treturn\n\n\tvar reloaded_layer := _reload("res://layer.tscn")\n\tif reloaded_layer == null:\n\t\t_fail("failed to reload TileMapLayer scene")\n\t\treturn\n\tvar got_layer := reloaded_layer.get_node_or_null("Layer") as TileMapLayer\n\tif got_layer == null:\n\t\t_fail("reloaded TileMapLayer missing")\n\t\treturn\n\tif got_layer.get_cell_source_id(Vector2i(1, 2)) != 0:\n\t\t_fail("TileMapLayer cell did not persist after reload")\n\t\treturn\n\tif got_layer.get_cell_atlas_coords(Vector2i(1, 2)) != Vector2i(0, 0):\n\t\t_fail("TileMapLayer atlas coords mismatch after reload")\n\t\treturn\n\n\tvar reloaded_map := _reload("res://map.tscn")\n\tif reloaded_map == null:\n\t\t_fail("failed to reload TileMap scene")\n\t\treturn\n\tvar got_map := reloaded_map.get_node_or_null("Map") as TileMap\n\tif got_map == null:\n\t\t_fail("reloaded TileMap missing")\n\t\treturn\n\tif got_map.get_cell_source_id(0, Vector2i(3, 4)) != 0:\n\t\t_fail("TileMap cell did not persist after reload")\n\t\treturn\n\tif got_map.get_cell_atlas_coords(0, Vector2i(3, 4)) != Vector2i(0, 0):\n\t\t_fail("TileMap atlas coords mismatch after reload")\n\t\treturn\n\n\tprint(JSON.stringify({"ok": true, "layer": true, "tilemap": true}))\n\treloaded_layer.queue_free()\n\treloaded_map.queue_free()\n\tquit(0)\n`);
    ...
    const output = `${run.stdout}\n${run.stderr}`;
    assert.match(output, /"ok"\s*:\s*true/, 'tilemap runner should report success JSON');
  } finally {
    rmSync(projectDir, { recursive: true, force: true });
  }
}
